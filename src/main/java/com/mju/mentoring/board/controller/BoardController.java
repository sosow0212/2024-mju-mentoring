...
public class BoardController {
    ...
    public ResponseEntity<BoardResponse> createBoard(@RequestBody BoardCreateRequest createRequest) {
        BoardResponse boardResponse = boardService.createBoard(createRequest);
        return ResponseEntity.ok(boardResponse); //게시글 저장 및 HTTP 상태코드 200과 함께 게시글 객체 반환
    }

    // R(Read) 게시글 조회 기능
    @GetMapping// /board 경로로 들어오는 GET 요청 처리
    public ResponseEntity<List<BoardListResponse>> getAllBoards() {
        List<BoardListResponse> boardResponses = boardService.findAll();
        return ResponseEntity.ok(boardResponses); //findAll()을 통해 조회된 게시글과 HTTP 상태코드 200 반환
    ...
    public ResponseEntity<BoardResponse> updateBoards(@PathVariable Long id, @RequestBody BoardUpdateRequest updateRequest) {
        BoardResponse updatedBoardResponse = boardService.updateBoard(id, updateRequest); //게시글 update
        return ResponseEntity.ok(updatedBoardResponse); //코드의 가독성을 높이기 위해서 메서드 체이닝은 한 줄에 하나씩만 사용한다. //HTTP 상태토드 200 반환
    }

    //D(Delete) 게시글 삭제 기능
    @DeleteMapping("/{id}")// /board/{id} 경로로 들어오는 DELETE 요청 처리
    public ResponseEntity<Void> deleteBoards(@PathVariable Long id) {
        boardService.deleteBoard(id); //게시글 delete
